...
CREATE PROCEDURE GetPrimaryKeyValues_2
...
BEGIN
    ...
    BEGIN
        ...
        SET @DynamicSQL = 'SELECT @PrimaryKeyValues = COALESCE(@PrimaryKeyValues + '', '', '''') + CONVERT(NVARCHAR(MAX), ' + QUOTENAME(@PrimaryKeyColumnName) + ') FROM ' + QUOTENAME(@TableName);

        -- Execute dynamic SQL
        EXEC sp_executesql @DynamicSQL, N'@PrimaryKeyValues NVARCHAR(MAX) OUTPUT', @PrimaryKeyValues OUTPUT;

        PRINT @PrimaryKeyValues;
...
EXEC GetPrimaryKeyValues_2 'ChildTable2', @result OUTPUT;
print @result;
...
CREATE PROCEDURE UPDATION_OF_DATA_PROPER_LOGS_FAULT_TOLERANCE_HEADER_CHILD_9
...
BEGIN
	...
	Create table #tempPrimary (logid int, tablename nvarchar(max), prim_keys nvarchar(max));
...
';
    ...
    EXEC sp_executesql @SQL, N'@LogsID INT', @LogsID;
			...
			-- Check if child table exists, if yes, Insert it
				...
				EXEC GetPrimaryKeyValues_2 ''' + @ChildTableName + ''', @prim_vals OUTPUT;
				INSERT INTO #tempPrimary (logid, tablename , prim_keys) VALUES (@LogsID, ''' + QUOTENAME(@ChildTableName) + ''', @prim_vals);
					';
	...
	END CATCH;
		...
		SET @SQL = '
			...
			INSERT INTO ' + QUOTENAME(@TargetDatabase) + '.' + QUOTENAME(@TargetSchema) + '.Child_Log_Table (LogID, TableName, PrimaryKeysMoved)
			SELECT logid, tablename, prim_keys FROM #tempPrimary;
...
exec UPDATION_OF_DATA_PROPER_LOGS_FAULT_TOLERANCE_HEADER_CHILD_9 'TestDB', 'dbo', 'ParentTable', 'TestDB_Backup', 'dbo', 'ParentTable', '', '','Just a sample try Again';