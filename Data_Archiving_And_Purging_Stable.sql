...
BEGIN
	...
	Create table #tempPrimary (logid int, childlogid int, tablename nvarchar(max), column_name nvarchar(max), datatype nvarchar(max), prim_keys nvarchar(max), no_of_prim int, columns_changed nvarchar(max));
...
';
    ...
    EXEC sp_executesql @SQL, N'@LogsID INT', @LogsID;
			...
			-- Check if child table exists, if yes, Insert it
				...
				DECLARE @ChangesInColumn NVARCHAR(MAX); -- Declare @ChangesInColumn inside dynamic SQL
				...
				UPDATE ' + QUOTENAME(@TargetDatabase) + '.' + QUOTENAME(@TargetSchema) + '.' + QUOTENAME('fix_table_2') + '
				SET Child_ID = @childid_new
				WHERE Child_ID = @childid;

				EXEC GetPrimaryKeyInfo_6 ''' + @ChildTableName + ''',
					@pri_col_val OUTPUT,
					@pri_col_datatype OUTPUT,
					@prim_vals OUTPUT,
					@prim_count OUTPUT;

				SET @ChangesInColumn = ''No Changes'';

				INSERT INTO #tempPrimary (logid, childlogid, tablename, column_name, datatype, prim_keys, no_of_prim, columns_changed)
				VALUES (@LogsID, @childid, ''' + QUOTENAME(@ChildTableName) + ''',
					@pri_col_val, @pri_col_datatype, @prim_vals, @prim_count, @ChangesInColumn);
			';
				EXEC sp_executesql @SQL, N'@LogsID INT', @LogsID;
				...
				SET @SQL = '
					...
					BEGIN CATCH
						...
						BEGIN TRY
							...
							Update #tempPrimary set columns_changed = ''Column Removed'' where logid = @LogsID;
						';
						EXEC sp_executesql @SQL, N'@LogsID INT', @LogsID;
						...
						Update #tempPrimary set columns_changed = ''Column Added'' where logid = @LogsID;
						';
...
SET @MailBody2 = 
	...
	END CATCH;
		...
		SET @SQL = '
			...
			INSERT INTO ' + QUOTENAME(@TargetDatabase) + '.' + QUOTENAME(@TargetSchema) + '.Child_Log_Table (LogID, Child_LogsID,TableName, ColumnName, DataType, PrimaryKeysMoved, No_of_Keys_Moved, Columns_Changed)
			SELECT logid, childlogid, tablename, column_name, datatype, prim_keys, no_of_prim, columns_changed FROM #tempPrimary;
